...
float LFO::processSample(float input)
{
    ...
    auto phaseIncrement = juce::MathConstants<float>::twoPi * frequency.getNextValue() / sampleRate;

    // Advance the phase and apply the phase offset, then center it around -pi to pi
    float currentPhase = phase.advance(phaseIncrement) - juce::MathConstants<float>::pi; // + phaseOffset;
